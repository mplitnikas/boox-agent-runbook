...
### 5.26 Termux killed minutes after backgrounding: the Onyx "full PM access" flag (2026-09-09/10)
Symptoms, in the order they were found: (a) `am_kill ... cached idle & background restricted` 2-10 min after backgrounding, with `cmd appops get com.termux` showing `Uid mode: RUN_ANY_IN_BACKGROUND: ignore` (Tailscale had it too); (b) after clearing that, `Force stopping com.termux ... from pid <system_server>` at a fixed second every N x 2 min, even while the package was not running, and with its foreground-service notification still up. Package stayed `enabled=0` throughout, so neither was the 5.16 freeze.
Root cause, from decompiling `/system/framework/framework.jar` (Onyx's `android.onyx.optimization.*` runs inside system_server; `jadx --single-class` on the pulled jar): every app has an EAC config (`adb shell dumpsys oec_service`, one `EACAppConfig:<pkg>{json}` line each) whose `extraConfig` carries `fullPMAccess` (default false) and `fullPMAccessTimeout` (default 300000 ms). Three things key off it:
- `EACPMImpl` sets the Android uid appop RUN_ANY_IN_BACKGROUND to *ignore* (= Settings > Battery > Restricted) at boot for every non-Onyx app with `fullPMAccess=false`, and on every config change for apps with `fullPMAccessTimeout==0`. That is what kept re-restricting Termux and Tailscale, and why `cmd appops set` without `--uid` looked ineffective (it only set the package mode; the uid mode wins).
- `EACAutoFreezeImpl` + `ApplicationFreezeController`: when an app with `autoFreezeConfig.supportAutoFreeze=true` leaves the foreground it goes into a map; a sweep every 2 min (`scheduleAtFixedRate`, hence the fixed second) calls `forceStopPackageWithoutPermissionCheck` + removes the recents task once it has been paused for `fullPMAccessTimeout`, unless it is the top task or has a foreground service (that check did not save Termux in practice). With `autoFreeze=true` it also disables the package (5.16). `supportAutoFreeze` was true for Termux, Termux:Boot and the Claude app (toggled individually in the freeze GUI) and false for Brave, Tailscale, Key Mapper (installed after "auto-freeze newly installed apps" was unticked), which is why only Termux and Claude were hit. `fullPMAccessTimeout < 0` and `autoFreeze=false` drops the app from the sweep entirely.
- `OnyxPowerManager`/`WakelockHelper`/`OnyxAlarmHelper`: wake locks and alarms from apps without full PM access are refused (`WakeLock tag:... from [com.termux] is forbidden` in logcat), so Termux's "acquire wakelock" was a no-op.
Fix: `~/bin/eink-bgexempt` (called by `eink-check`) reads the live config from dumpsys, sets `extraConfig.fullPMAccess=true` and `fullPMAccessTimeout=-1` for com.termux, com.termux.boot (shares Termux's uid, so it must match), com.tailscale.ipn and com.anthropic.claude, and writes it back with `service call oec_service 12` (`IOECService.applyAppConfigToService(List<String> json, Bundle)`; the trailing `i32 1 i32 0` is a non-null empty Bundle, a null one NPEs). No permission check, shell can call it; the service persists to MMKV and the change survived a reboot. The uid appop went back to allow immediately and stayed allow across the reboot; wake locks are allowed after the next reboot (the PM allowlist only reloads on a data-changed broadcast the setter does not send). Originals from the dump are kept in the session scratch (`oec-revert.sh`) and can be regenerated from any old `dumpsys oec_service` capture.
Side effect to expect: applying a config makes Onyx `killAppForConfigChange` every app in the list that is not on top. Tailscale restarted itself (always-on VPN) but came back with its MagicDNS resolver dead (`netstack inject inbound: write /dev/tun: input/output error` after the tun0->tun1 handoff): 100.100.100.100 answered pings but not DNS, so every name lookup on the tablet hung, including `ssh lap`. A reboot reproduced it; `am force-stop com.tailscale.ipn` + `am start -n com.tailscale.ipn/.MainActivity` (force-stopped apps are not revived by always-on VPN) gave a clean single establish and DNS returned. To stop `lap` depending on MagicDNS at all, Termux `~/.ssh/config` `Host lap` now uses the laptop's Tailscale IP (known_hosts entry added; `Host lapdns` keeps the name). No trailing comments in ssh_config, it is not supported.
Still GUI-only: nothing new. `eink-check` now covers freeze-sweep/appop/wake-lock exemptions, the uid appop, and the doze whitelist; the Onyx App Startup allowlist and the auto-freeze toggles remain GUI.
Addendum (2026-09-10 01:00): the Caps Lock layout reset at boot (5.24) has the same root. The ExKeyMo layout package had `autoFreezeConfig.autoFreeze=true`, so `EACAutoFreezeImpl.initFreezeList` disabled it on every boot before Android restored the per-keyboard layout selection. `eink-bgexempt` now also sets `autoFreeze=false` (plus the PM fields) for `ris58h.exkeymo_keyboard_layout`; if the selection survives the next reboot, `eink-capslock` becomes a fallback only. The GUI route cannot run without taking the screen: taps and uiautomator dumps go to the visible display, and a virtual display would need root or an overlay that shows on the e-ink panel.

### 5.27 Codex composer unreadable on e-ink: OSC 11 background detection (2026-09-12)
Symptom after the Codex CLI updated to 0.154.0: the composer (text input box) in the Codex pane renders as a solid black block on the tablet, and the placeholder row grows animated braille "twinkling stars".

Cause, from driving the TUI in a pty and diffing the cell attributes (`pyte` over the raw stream):
- Codex 0.154 fills the composer with a background colour **derived from the terminal background it detects**, and leaves the input text at the terminal default foreground. It learns the background by sending `OSC 11` (`ESC ] 11 ; ? ESC \`) **once, at startup** — a later resize or reattach does not re-detect (verified: one query during boot, none after a `SIGWINCH` + resize).
- Answer white `#ffffff` → fill `#f4f4f4`. Answer catppuccin `#1e1e2e` → fill `#393947`. No answer at all → **no fill**, plain text.
- On the tablet the terminal default foreground is black (5.12), so a dark-detected fill is black-on-dark-grey — invisible.
- herdr **forwards** the pane's OSC 11 query to the attached host terminal; it does not answer from its own theme. Verified with an isolated herdr (own `HERDR_CONFIG_PATH` + `HERDR_SOCKET_PATH`, `[experimental] allow_nested = true`) running `[theme] name = "catppuccin"` while the harness answered white: Codex still chose the light fill. So 5.22's `auto_switch` is irrelevant here, and changing the herdr theme does not fix it.
- The trigger is therefore *where the pane was started*: a Codex launched while a dark laptop terminal was attached keeps the dark fill forever, including when you later attach from Termux. (A pane restored with no client attached gets no answer, so it renders flat — that case was always fine.)

Dead ends, all verified: `tui.theme` only selects the **syntax highlighting** theme (`/theme` says so itself; a custom white `.tmTheme` in `$CODEX_HOME/themes/` changed nothing about the composer). `COLORFGBG` is read by `codex doctor` but not used for detection. `TERM=xterm` still fills.

Fix, laptop side, two parts:
1. `~/.zshrc`: `codex() { NO_COLOR=1 command codex "$@"; }`. With `NO_COLOR` set, Codex draws **no composer fill** whatever it detected, so the same process reads correctly on both a white e-ink terminal and a dark laptop one. Cost: no colour anywhere in Codex output, diffs included — accepted because the tablet is the primary Codex surface and colour-only distinctions are not wanted on e-ink anyway. Remove the `NO_COLOR` to get colour back.
...
Evidence level: driven in a pty with a fake host terminal and, end to end, inside an isolated herdr whose host answer was forced dark — with both fixes in place the composer came back with no fill and no braille. Not yet confirmed on the physical tablet.
